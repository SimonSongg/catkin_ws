  ...
  initialized_(false)
{
  ...
  imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(
    //ros::names::resolve("mobile_base") + "/sensors/imu_data", 5);
    ros::names::resolve("imu") + "/data/fuse", 5);

  // imu_ypr_publisher_ = nh_.advertise<geometry_msgs::Vector3>(
  //   ros::names::resolve("imu") + "/data/ypr", 5);
  ...
  imu_subscriber_.reset(new ImuSubscriber(
    nh_, ros::names::resolve("zed2") + "/zed_node/imu/data", queue_size));
  ...
  if (use_mag_)
  {
    if (use_magnetic_field_msg_)
    {
      mag_subscriber_.reset(new MagSubscriber(
        nh_, ros::names::resolve("/zed2") + "/zed_node/imu/mag", queue_size));
    }
    else
    {
      mag_subscriber_.reset(new MagSubscriber(
...
void ImuFilterRos::publishFilteredMsg(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  // boost::shared_ptr<geometry_msgs::Vector3> imu_ypr_msg;

  // tf2::Quaternion imu_quat_tf(q0, q1, q2, q3);
  // double roll, pitch, yaw;

  // tf2::Matrix3x3(imu_quat_tf).getRPY(roll, pitch, yaw);
  ...
  // imu_ypr_publisher_.publish(imu_ypr_msg);
  ...
  if(publish_debug_topics_)
  {
    ...
    rpy.vector.x = rpy.vector.x / 3.14159265358979323 * 180;
    rpy.vector.y = rpy.vector.y / 3.14159265358979323 * 180;
    rpy.vector.z = rpy.vector.z / 3.14159265358979323 * 180;
...
{
  geometry_msgs::Vector3Stamped rpy;
  rpy.vector.x = roll / 3.14159265358979323 * 180;
  rpy.vector.y = pitch / 3.14159265358979323 * 180;
  rpy.vector.z = yaw / 3.14159265358979323 * 180;
  rpy.header.stamp = t;
